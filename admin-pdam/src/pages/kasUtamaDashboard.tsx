...
import { getKasUtamaDashboard } from "../service/kasUtama.service"
...
export default function KasUtamaDashboard() {
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true)
      const response = await getKasUtamaDashboard()
      setData(response || initialData)
    } catch (error) {
      console.error(error)
      toast.error("Gagal memuat data dashboard kas utama")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="rounded-2xl border border-gray-200 bg-gradient-to-br from-blue-50 via-white to-cyan-50 p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <h1 className="text-2xl font-bold text-gray-900">Dashboard Kas Utama</h1>
