...
import * as kasUtamaService from "../service/kasUtama.service"
...
const initialFormValues: KasUtamaFormValues = {
  tanggal: "",
  jenis: "masuk",
  kategori: "",
  deskripsi: "",
...
export default function KasUtamaPage() {
  const { user } = useAuth()
  const canCRUD = canCRUDPath(user?.role, "/kas-utama")
  
  const [rows, setRows] = useState<KasUtamaRow[]>([])
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [modalMode, setModalMode] = useState<"create" | "edit">("create")
  const [selectedId, setSelectedId] = useState<number | null>(null)
  const [selectedRow, setSelectedRow] = useState<KasUtamaRow | null>(null)
  const [formValues, setFormValues] =
    useState<KasUtamaFormValues>(initialFormValues)
  ...
  const columns = useMemo(
    () => [
      ...
      { header: "Deskripsi", accessor: "deskripsi" },
      {
        ...
        accessor: "jumlah",
        sortable: true,
        render: (value: number, row: KasUtamaRow) =>
          row.jenis === "masuk" ? (
  ...
  const fetchData = async () => {
    try {
      const response = await kasUtamaService.getKasUtama()
      ...
      console.error(error)
      toast.error("Gagal memuat data kas utama")
    }
  }
  ...
  const handleOpenEdit = (row: KasUtamaRow) => {
    setModalMode("edit")
  ...
  const handleOpenDeleteDialog = (row: KasUtamaRow) => {
    setSelectedRow(row)
  ...
  const executeSubmit = async () => {
    ...
    try {
      ...
      if (modalMode === "create") {
        await kasUtamaService.createKasUtama(payload)
        toast.success("Kas Utama berhasil ditambahkan")
      } else if (selectedId !== null) {
        await kasUtamaService.updateKasUtama(selectedId, payload)
        toast.success("Kas Utama berhasil diperbarui")
      }
      ...
      console.error(error)
      toast.error("Gagal menyimpan kas utama")
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await kasUtamaService.deleteKasUtama(selectedRow.id)
      toast.success("Kas Utama berhasil dihapus")
      setIsDeleteDialogOpen(false)
      setSelectedRow(null)
      await fetchData()
    } catch (error) {
      console.error(error)
      toast.error("Gagal menghapus kas utama")
    }
  }
  ...
  return (
    <>
      ...
      <FormModal
        open={isModalOpen}
        mode={modalMode}
        title={modalMode === "create" ? "Tambah Kas Utama" : "Edit Kas Utama"}
        fields={modalFields}
        values={formValues}
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={handleDeleteDialogChange}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-gray-600">
            Apakah kamu yakin ingin menghapus kas utama kategori{" "}
      ...
      <Dialog open={isSubmitDialogOpen} onOpenChange={setIsSubmitDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-gray-600">
            {modalMode === "create"
              ? "Apakah kamu yakin ingin menambahkan kas utama ini?"
              : "Apakah kamu yakin ingin menyimpan perubahan kas utama ini?"}
          </p>
          <DialogFooter>
            <button
              type="button"
